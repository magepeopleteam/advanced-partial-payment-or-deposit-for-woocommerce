...
function send_partially_paid_order_email_notification($order_id) {
    ...
    // Check if the order status is "partially-paid"
    if ($order && $order->get_status() === 'partially-paid') {
...
function display_payment_history_on_thankyou($order_id) {
    $order = wc_get_order($order_id);
    if (!$order) return;
  // Retrieve the amount type from order meta
  $amount_type = get_post_meta($order_id, '_mepp_amount_type', true);
// Check the amount type and conditionally display payment history
if ($amount_type === 'fixed' || $amount_type === 'percent' || $amount_type === 'payment_plan') {
    // Exit if the amount type is one of the specified values
    return;
}
    ...
    foreach ($items as $item) {
        ...
        // Get order meta for additional payments
