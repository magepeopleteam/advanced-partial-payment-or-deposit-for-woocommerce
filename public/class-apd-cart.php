...
class APD_Cart {
	...
	public function display_cart_item_deposit( $cart_item, $cart_item_key ) {
		...
		echo '<small class="apd-cart-deposit-tag">' . esc_html( $deposit_label ) . ': ' . wp_kses_post( wc_price( $deposit ) ) . '</small>';
		echo '</div>';
	}
	...
	public function cart_item_subtotal( $subtotal, $cart_item, $cart_item_key ) {
		...
		return $subtotal . '<br><small class="apd-subtotal-deposit">' . esc_html( $deposit_label ) . ': ' . wp_kses_post( wc_price( $total_dep ) ) . '</small>';
	}
