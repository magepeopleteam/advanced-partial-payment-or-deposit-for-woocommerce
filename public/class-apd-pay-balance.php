...
class APD_Pay_Balance {
	...
	public function handle_pay_balance() {
		...
		$amount = APD_Order::sanitize_balance_payment_amount(
			$order,
			isset( $_GET['apd_amount'] ) ? wc_format_decimal( wp_unslash( $_GET['apd_amount'] ) ) : 0 // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wc_format_decimal() is the sanitizer; it also handles locale decimal separators, which floatval() would break.
		);
	...
	public function create_balance_order() {
		...
		$amount = APD_Order::sanitize_balance_payment_amount(
			$order,
			isset( $_POST['amount'] ) ? wc_format_decimal( wp_unslash( $_POST['amount'] ) ) : 0 // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wc_format_decimal() is the sanitizer; it also handles locale decimal separators, which floatval() would break.
		);
