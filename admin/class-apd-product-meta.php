					...
					>
						...
						<div id="apd-summary-plans-section" style="margin-top:12px;padding:14px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">
							...
							<?php if ( ! empty( $active_plans ) ) : ?>
								<?php
								foreach ( $active_plans as $pid => $plan ) :
									$total_pct = 0;
									foreach ( $plan['installments'] as $inst ) {
										$total_pct += floatval( $inst['amount'] );
									}
									$plan_status_active = ( $plan['status'] ?? 'active' ) === 'active';
									?>
								<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;border:1px solid #e2e8f0;border-radius:6px;background:#fff;margin-bottom:6px;">
									<div>
										...
										<span style="color:#94a3b8;font-size:11px;margin-left:6px;">
											<?php echo esc_html( count( $plan['installments'] ) ); ?> <?php esc_html_e( 'installments', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?>
										</span>
									</div>
									<div>
										<span style="background:#eef2ff;color:#4f46e5;padding:2px 8px;border-radius:4px;font-size:11px;font-weight:600;">
											<?php echo esc_html( $total_pct ); ?><?php echo esc_html( 'percentage' === $plan['price_type'] ? '%' : ' ' . get_woocommerce_currency_symbol() ); ?>
										</span>
										<span style="background:<?php echo esc_attr( $plan_status_active ? '#d1fae5' : '#f1f5f9' ); ?>;color:<?php echo esc_attr( $plan_status_active ? '#065f46' : '#64748b' ); ?>;padding:2px 8px;border-radius:4px;font-size:11px;font-weight:600;margin-left:4px;">
