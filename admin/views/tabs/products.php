...
<div class="apd-card">
	...
	<div class="apd-card-body">
		<?php
		$products_with_deposit = get_posts(
			array(
				'post_type'   => 'product',
				'numberposts' => 20,
				'meta_query'  => array(
		...
		<table class="apd-table">
			...
			<tbody>
				<?php
				foreach ( $products_with_deposit as $product_post ) :
					$deposit_type = get_post_meta( $product_post->ID, '_apd_deposit_type', true );
					$value        = get_post_meta( $product_post->ID, '_apd_deposit_value', true );
					?>
				<tr>
					<td><strong><?php echo esc_html( $product_post->post_title ); ?></strong></td>
					<td><span class="apd-badge"><?php echo esc_html( ucfirst( $deposit_type ) ); ?></span></td>
					<td><?php echo esc_html( $value ); ?><?php echo esc_html( 'percentage' === $deposit_type ? '%' : ' ' . get_woocommerce_currency_symbol() ); ?></td>
					<td>
						<a href="<?php echo esc_url( get_edit_post_link( $product_post->ID ) ); ?>" class="apd-btn apd-btn-small">
