		...
		<table class="apd-table" id="apd-category-rules-table">
			...
			<tbody>
				...
				foreach ( $product_cats as $category ) :
					$enabled      = get_term_meta( $category->term_id, '_apd_enable_deposit', true );
					$deposit_type = get_term_meta( $category->term_id, '_apd_deposit_type', true );
					$value        = get_term_meta( $category->term_id, '_apd_deposit_value', true );

					if ( empty( $enabled ) && empty( $deposit_type ) ) {
						continue;
					}
					$has_rules = true;
					?>
				<tr data-cat-id="<?php echo esc_attr( $category->term_id ); ?>">
					<td><strong><?php echo esc_html( $category->name ); ?></strong></td>
					<td>
						<span class="apd-status-dot <?php echo esc_attr( 'yes' === $enabled ? 'apd-status-active' : 'apd-status-inactive' ); ?>"></span>
						<?php echo 'yes' === $enabled ? esc_html__( 'Active', 'advanced-partial-payment-or-deposit-for-woocommerce' ) : esc_html__( 'Inactive', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?>
					</td>
					<td><span class="apd-badge"><?php echo esc_html( ucfirst( $deposit_type ?: 'global' ) ); ?></span></td>
					<td><?php echo esc_html( $value ); ?><?php echo esc_html( 'percentage' === $deposit_type ? '%' : ( $value ? ' ' . get_woocommerce_currency_symbol() : '' ) ); ?></td>
