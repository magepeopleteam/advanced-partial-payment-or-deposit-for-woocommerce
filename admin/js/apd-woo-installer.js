...
(function ($) {
	...
	var config    = window.apd_woo_installer || {};
	var $overlay  = null;
	var $popup    = null;
	var $btn      = null;
	var $progress = null;
	...
	$(document).ready(function () {
		$overlay  = $('#apd-woo-overlay');
		$popup    = $overlay.find('.apd-woo-popup');
		$btn      = $('#apd-woo-install-btn');
		$progress = $('#apd-woo-progress');
		$fill     = $('#apd-woo-progress-fill');
		$status   = $('#apd-woo-status-text');
		$actions  = $overlay.find('.apd-woo-actions');
	...
	function installWooCommerce() {
		$.ajax({
			...
			dataType: 'json',
			data: {
				action: 'apd_install_woocommerce',
	...
	function activateWooCommerce() {
		$.ajax({
			...
			dataType: 'json',
			data: {
				action: 'apd_activate_woocommerce',
	...
	function setProgress(percent, text) {
		...
		$status.text(text).removeClass('apd-success apd-error');
	}
	...
	function showSuccess() {
		...
		$popup.addClass('apd-state-success');
		$status.addClass('apd-success');

		// Change icon to checkmark
		$popup.find('.apd-woo-icon').html(
		...
		// Update title
		$popup.find('.apd-woo-title').text(config.i18n.success);
		$popup.find('.apd-woo-desc').text(config.i18n.redirecting);
	...
	function showError(message) {
		isWorking = false;
		$popup.addClass('apd-state-error');
		$status.text(message).addClass('apd-error');
		$fill.css('width', '100%');
		...
		setTimeout(function () {
			$popup.removeClass('apd-state-error');
