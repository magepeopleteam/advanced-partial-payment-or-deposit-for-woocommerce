...
class APD_Admin_Order {
	...
	public function add_deposit_metabox() {
		...
		add_meta_box(
			'apd-deposit-details',
			__( '💰 Deposit Details', 'advanced-partial-payment-or-deposit-for-woocommerce' ),
		...
		add_meta_box(
			...
			__( 'Deposit Payment Record', 'advanced-partial-payment-or-deposit-for-woocommerce' ),
	...
	public function render_deposit_metabox( $post_or_order ) {
		...
		<div class="apd-metabox-content">
			<table class="apd-metabox-table" style="width:100%;border-collapse:collapse;">
				<tr>
					<td style="padding:8px 0;color:#666;"><?php esc_html_e( 'Full Order Total', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></td>
					<td style="padding:8px 0;text-align:right;font-weight:600;"><?php echo wp_kses_post( wc_price( $details['total_amount'] ) ); ?></td>
				</tr>
				<tr>
					<td style="padding:8px 0;color:#666;"><?php esc_html_e( 'Deposit Amount', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></td>
					<td style="padding:8px 0;text-align:right;color:#2271b1;font-weight:600;"><?php echo wp_kses_post( wc_price( $details['deposit_amount'] ) ); ?></td>
				</tr>
				<tr>
				...
				<tr style="border-top:2px solid #e2e4e7;">
					...
					<td style="padding:10px 0;text-align:right;font-weight:700;color:<?php echo esc_attr( $details['balance_due'] > 0 ? '#d63638' : '#00a32a' ); ?>;font-size:16px;">
						<?php echo wp_kses_post( wc_price( $details['balance_due'] ) ); ?>
					</td>
				</tr>
			</table>
			...
			<div style="margin-top:12px;padding-top:12px;border-top:1px solid #e2e4e7;">
				<p style="margin:0 0 8px;font-weight:600;font-size:12px;text-transform:uppercase;color:#666;">
					<?php esc_html_e( 'Record Manual Payment', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?>
				</p>
				<div style="display:flex;gap:6px;">
					...
					<button type="button" class="button button-primary" id="apd-record-payment"
						...
						<?php esc_html_e( 'Record', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?>
					</button>
				</div>
			</div>
			<?php endif; ?>
			...
			<div style="margin-top:12px;padding-top:12px;border-top:1px solid #e2e4e7;">
				<p style="margin:0 0 8px;font-weight:600;font-size:12px;text-transform:uppercase;color:#666;">
					<?php esc_html_e( 'Payment History', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?>
				</p>
				...
				<div style="padding:8px 0;border-bottom:1px solid #f0f0f1;">
					<div style="display:flex;justify-content:space-between;gap:8px;font-size:12px;color:#555;">
						<span style="font-weight:600;color:#1d2327;">
							<?php /* translators: %d: payment number in the sequence. */ echo esc_html( sprintf( __( 'Payment %d', 'advanced-partial-payment-or-deposit-for-woocommerce' ), $index + 1 ) ); ?>
						</span>
						<span style="font-weight:600;"><?php echo wp_kses_post( wc_price( floatval( $entry['amount'] ?? 0 ) ) ); ?></span>
					</div>
	...
	public function render_payment_record_metabox( $post_or_order ) {
		...
		if ( ! $order || ! APD_Order::is_deposit_order( $order ) ) {
			echo '<p style="color:#999;">' . esc_html__( 'No deposit payment record available for this order.', 'advanced-partial-payment-or-deposit-for-woocommerce' ) . '</p>';
			return;
		}

		$details = APD_Order::get_deposit_details( $order );
		if ( ! $details ) {
			echo '<p style="color:#999;">' . esc_html__( 'No payment record found.', 'advanced-partial-payment-or-deposit-for-woocommerce' ) . '</p>';
			return;
		}
		...
		<div class="apd-admin-payment-record">
			<div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:16px;">
				<div style="padding:12px 14px;border:1px solid #e2e4e7;border-radius:8px;background:#fff;">
					<div style="font-size:11px;font-weight:600;color:#646970;text-transform:uppercase;"><?php esc_html_e( 'Full Order Total', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></div>
					<div style="margin-top:6px;font-size:18px;font-weight:700;color:#1d2327;"><?php echo wp_kses_post( wc_price( $details['total_amount'] ) ); ?></div>
				</div>
				<div style="padding:12px 14px;border:1px solid #e2e4e7;border-radius:8px;background:#fff;">
					<div style="font-size:11px;font-weight:600;color:#646970;text-transform:uppercase;"><?php esc_html_e( 'Deposit Amount', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></div>
					<div style="margin-top:6px;font-size:18px;font-weight:700;color:#2271b1;"><?php echo wp_kses_post( wc_price( $details['deposit_amount'] ) ); ?></div>
				</div>
				<div style="padding:12px 14px;border:1px solid #e2e4e7;border-radius:8px;background:#fff;">
				...
				<div style="padding:12px 14px;border:1px solid #e2e4e7;border-radius:8px;background:#fff;">
					...
					<div style="margin-top:6px;font-size:18px;font-weight:700;color:<?php echo esc_attr( $details['balance_due'] > 0 ? '#d63638' : '#00a32a' ); ?>;"><?php echo wp_kses_post( wc_price( $details['balance_due'] ) ); ?></div>
				</div>
			</div>
			...
			<table class="widefat striped" style="border:1px solid #e2e4e7;">
				<thead>
					<tr>
						<th style="width:80px;"><?php esc_html_e( '#', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></th>
						<th style="width:160px;"><?php esc_html_e( 'Payment', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></th>
						<th style="width:180px;"><?php esc_html_e( 'Date', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></th>
						<th><?php esc_html_e( 'Note', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></th>
						<th style="width:140px;text-align:right;"><?php esc_html_e( 'Amount', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></th>
						<th style="width:160px;text-align:right;"><?php esc_html_e( 'Running Paid', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></th>
						<th style="width:160px;text-align:right;"><?php esc_html_e( 'Balance After', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php if ( empty( $history ) ) : ?>
						<tr>
							<td colspan="7"><?php esc_html_e( 'No payment entries recorded yet.', 'advanced-partial-payment-or-deposit-for-woocommerce' ); ?></td>
						</tr>
					<?php else : ?>
						<?php foreach ( $history as $index => $entry ) : ?>
							...
							?>
							<tr>
								<td><strong><?php /* translators: %d: payment number in the sequence. */ echo esc_html( sprintf( __( 'Payment %d', 'advanced-partial-payment-or-deposit-for-woocommerce' ), $index + 1 ) ); ?></strong></td>
								<td><?php echo esc_html( $this->get_payment_type_label( $entry ) ); ?></td>
								<td><?php echo esc_html( $entry['date'] ?? '' ); ?></td>
								<td><?php echo esc_html( $entry['note'] ?? '' ); ?></td>
								<td style="text-align:right;font-weight:600;"><?php echo wp_kses_post( wc_price( $amount ) ); ?></td>
								<td style="text-align:right;"><?php echo wp_kses_post( wc_price( $running_total ) ); ?></td>
								<td style="text-align:right;color:<?php echo esc_attr( $balance_after > 0 ? '#d63638' : '#00a32a' ); ?>;font-weight:600;"><?php echo wp_kses_post( wc_price( $balance_after ) ); ?></td>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
				</tbody>
	...
	public function add_order_columns( $columns ) {
		...
		foreach ( $columns as $key => $label ) {
			...
			if ( 'order_total' === $key ) {
				$new_columns['apd_deposit_status'] = __( 'Deposit', 'advanced-partial-payment-or-deposit-for-woocommerce' );
			}
		}
		return $new_columns;
	}
	...
	public function ajax_record_payment() {
		...
		$result = APD_Order::record_payment( $order_id, $amount, __( 'Manual payment recorded by admin', 'advanced-partial-payment-or-deposit-for-woocommerce' ) );

		if ( $result ) {
			wp_send_json_success( __( 'Payment recorded!', 'advanced-partial-payment-or-deposit-for-woocommerce' ) );
		} else {
			wp_send_json_error( __( 'Failed to record payment.', 'advanced-partial-payment-or-deposit-for-woocommerce' ) );
		}
	}
	...
	private function get_payment_type_label( $entry ) {
		...
		switch ( $type ) {
			case 'deposit':
				return __( 'Initial Deposit', 'advanced-partial-payment-or-deposit-for-woocommerce' );
			case 'balance_payment':
				return __( 'Remaining Balance', 'advanced-partial-payment-or-deposit-for-woocommerce' );
			default:
				return __( 'Payment', 'advanced-partial-payment-or-deposit-for-woocommerce' );
		}
	}
}
